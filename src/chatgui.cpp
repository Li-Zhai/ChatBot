...
ChatBotFrame::ChatBotFrame(const wxString &title) : wxFrame(NULL, wxID_ANY, title, wxDefaultPosition, wxSize(width, height)) {
    ...
    int idTextXtrl = 1;
    _userTextCtrl =
        new wxTextCtrl(ctrlPanel, idTextXtrl, "", wxDefaultPosition, wxSize(width, 50), wxTE_PROCESS_ENTER, wxDefaultValidator, wxTextCtrlNameStr);
...
void ChatBotFrame::OnEnter(wxCommandEvent &WXUNUSED(event)) {
    ...
    _panelDialog->GetChatLogicHandle()->SendMessageToChatbot(string(userText.mb_str()));
}
...
ChatBotPanelDialog::ChatBotPanelDialog(wxWindow *parent, wxWindowID id) : wxScrolledWindow(parent, id) {
    ...
    // !Task 1: make _chatLogic an exclusive resource to class ChatbotPanelDialog using
...
ChatBotPanelDialog::~ChatBotPanelDialog() {
    ...
    // delete _chatLogic;
    // !Task 1: no need to manually de-allocate
...
void ChatBotPanelDialog::PrintChatbotResponse(string response) {
...
ChatBotPanelDialogItem::ChatBotPanelDialogItem(wxPanel *parent, wxString text, bool isFromUser)
    ...
    _chatBotImg = new wxStaticBitmap(this, wxID_ANY, (isFromUser ? wxBitmap(imgBasePath + "user.png", wxBITMAP_TYPE_PNG) : *bitmap), wxPoint(-1, -1),
                                     wxSize(-1, -1));
